interface Pet {
  ...
  let lion: Animal;
  // leopard inferred type is { name: string; age: number; }
  let leopard = { name: "Leo", age: 23 };//In this case, lion must have a member called name that is a string. 
                                        //It does, so the assignment is allowed.
  lion = leopard; //No Error
...
let myObj = {name: "aa", baz: 1}

let myObj1 = {baz: 12, name: "bb"}

myObj = myObj1 // No Error

/*
let w = () => ({ name: "Alice" });
let m = () => ({ name: "Alice", location: "Seattle" });
w= m; // OK
m = w; // Error, because w() lacks a location property

let x1 = (a: number) => 0;
let y1 = (b: number, s: string) => 0;
y1 = x1; // OK
x1 = y1; // Error, because x1 lack a s parameter
*/